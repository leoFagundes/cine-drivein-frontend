    ...
    return (
        <Admin sideBar={false}>
            <section className={style.profileContainer}>
                <FaArrowLeft size={20} className={style.arrowBack} onClick={() => navigate('/admin')} />
                <div className={style.infoTemplate}>
                    <div className={style.userInfo}>
                        <h2>{user.username}</h2>
                        <p>{user.email}</p>
                        <p className={style.userAdminText}><i>Usuário Administrador</i> {user.isAdmin ? <FaCheck size={16} color='green' /> : <MdClose size={16} color='red' />}</p>
                    </div>
                    {user.profileImage !== '' ? (
                        <img
                ...
                <div className={style.avatarTemplate}>
                    {avatarData.map(({ seed }, index) => (
                        <div key={index} className={style.avatarSingle}>
                            {seed === '' ? (
                                <div onClick={() => handleClick(seed)} className={style.avatarImage}>
                                    <HiUser size={60} />
                                </div>
                            ) : (
                                <img
