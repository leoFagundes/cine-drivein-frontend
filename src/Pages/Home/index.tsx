...
export default function Home() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                console.error('Erro ao obter dados do horário de funcionamento:', error);
            }
        };
        fetchData();
    }, [])
    ...
    const handleSubmit = () => {
        ...
        if (name === '' || phone === '' || (spot.trim().length !== 3 && spot.trim().length !== 4)) {
        ...
        return (
            <section className={style.homeContainer}>
                <form className={style.formContainer}>
                    <LogoDrivein />
                    <p className={style.subtitleText}>Para fazer seu pedido, preencha os campos abaixo</p>
                    <InputPattern
                        value={name}
                    ...
                    <ButtonPattern mainText='Ir para o cardápio' type="button" onClick={() => handleSubmit()} />
                    <span onClick={openWhatsApp} className={style.helpLink}>Precisa de Ajuda? <IoLogoWhatsapp size={16} color='green' /></span>
                </form>
            </section>
        )
    } else {
            ...
            <section className={style.homeContainer}>
                <div className={style.isClose}>
                    ...
                    <div className={style.infoSchedule}>
                        ...
                        <p>de <strong>{openingTime}</strong> até <strong>{closingTime}</strong></p>
                    </div>
